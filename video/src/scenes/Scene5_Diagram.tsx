...
/**
 * Scene 5 — Diagrams on demand.
 * Reworked to look like a real Mermaid flowchart (what ULBS Coach actually
 * renders): rounded-rect nodes + decision diamonds + labelled arrows that draw
 * themselves in (nodes spring, edges reveal via stroke-dashoffset). Sits inside
 * a frosted-glass chat frame; the "camera" pulls back as it builds.
 */
export const Scene5Diagram: React.FC = () => {
  ...
  const camScale = interpolate(pull, [0, 1], [1.08, 1.0]);

  const DRAW = 40; // when the diagram starts building

  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center" }}>
      <div style={{ position: "absolute", top: 70 }}>
        <FadeSlide direction="up" distance={30} enterAt={8} enterDuration={18}>
          <div style={{ fontFamily: fonts.display, fontSize: 50, fontWeight: 700, letterSpacing: "-0.03em", color: brand.text, textAlign: "center" }}>
      ...
      <div style={{ transform: `scale(${camScale})`, width: 1120 }}>
        <GlassCard radius={26} padding={36} gradientEdge>
          <div style={{ marginBottom: 26 }}>
            <ChatBubble role="user" text="Explain binary search as a diagram." fontSize={24} maxWidth={560} />
          </div>
...
const Node: React.FC<NodeProps> = ({ frame, at, x, y, w, h, label, diamond, accent }) => {
  // Spring pop-in (manual, frame-based).
  ...
  return (
    <g opacity={eased} transform={`translate(${cx} ${cy}) scale(${scale}) translate(${-cx} ${-cy})`}>
      ...
      <text x={cx} y={cy + 7} textAnchor="middle" fill={brand.text} fontSize={21} fontFamily={fonts.body} fontWeight={500}>
        {label}
      </text>
    </g>
  );
...
const Edge: React.FC<EdgeProps> = ({ frame, at, d, label, lx, ly }) => {
  const draw = interpolate(frame, [at, at + 14], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const headOpacity = interpolate(frame, [at + 12, at + 18], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <g>
      <path
        ...
        markerEnd="url(#arrow)"
        opacity={headOpacity > 0 ? 1 : 0.85}
      />
      {label && (
        <g opacity={labelOpacity}>
          <rect x={(lx ?? 0) - 18} y={(ly ?? 0) - 16} width={36} height={26} rx={6} fill="#15131c" stroke="rgba(124,106,247,0.3)" strokeWidth={1} />
          <text x={lx} y={(ly ?? 0) + 2} textAnchor="middle" fill={brand.textMuted} fontSize={15} fontFamily={fonts.body}>
            {label}
          </text>
        </g>
      )}
...
const Flowchart: React.FC<{ frame: number; drawStart: number }> = ({ frame, drawStart }) => {
  const s = drawStart;
  return (
    <svg viewBox="0 0 1040 600" width="100%" style={{ display: "block" }}>
      ...
      {/* Edges (draw after their source node). */}
